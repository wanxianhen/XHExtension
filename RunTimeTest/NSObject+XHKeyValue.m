...
@implementation NSObject (XHKeyValue)
#pragma mark - 公共方法
#pragma mark - 字典转模型
/**
 *  通过字典来创建一个模型
...
- (void)setKeyValues:(NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^( XHIvar *ivar, BOOL *stop) {
        // 来自Foundation框架的成员变量，直接返回(如果是根类NSObject，以NS开头，class_copyIvarList返回ivar的属性名是isa指针，直接忽略)
...
- (NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^( XHIvar *ivar, BOOL *stop) {
//        if (ivar.isSrcClassFromFoundation) return;(如果是根类NSObject，以NS开头，class_copyIvarList返回ivar的属性名是isa指针，直接忽略)
        if ([ivar.name isEqualToString:@"isa"]) return ;
...
+ (NSArray *)keyValuesArrayWithObjectArray:(NSArray *)objectArray
{
    ...
    if (![objectArray isKindOfClass:[NSArray class]]) {
        [NSException raise:@"objectArray is not a NSArray" format:@"objectArray is not a NSArray"];
    }
...
+ (NSArray *)objectArrayWithKeyValuesArray:(NSArray *)keyValuesArray
{
    ...
    if (![keyValuesArray isKindOfClass:[NSArray class]]) {
        [NSException raise:@"keyValuesArray is not a NSArray" format:@"keyValuesArray is not a NSArray"];
    }
